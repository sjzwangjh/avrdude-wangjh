...
/*
 * We need to import enough from the JTAG ICE mkII definitions to be able to
 * talk to the ICE, query some parameters etc. The macro
 * JTAGMKII_PRIVATE_EXPORTED limits the amount of definitions that
 * jtagmkII_private.h will export, so to avoid conflicts with those names that
 * are identical to the STK500v2 ones.
 */
#include "jtagmkII.h"
...
/*
 * These two tables can be semi-automatically updated from targetboards.xml
 * using tools/get-stk600-cards.xsl.
 */
...
static void stk500v2_jtagmkII_setup(PROGRAMMER *pgm) {
   ...
   * our own cookie in a safe place for the time being.
   */
  mycookie = pgm->cookie;
  jtagmkII_setup(pgm);
...
static void stk500v2_jtag3_setup(PROGRAMMER *pgm) {
   ...
   * own cookie in a safe place for the time being.
   */
  mycookie = pgm->cookie;
  jtag3_setup(pgm);
  theircookie = pgm->cookie;
...
/*
 * Send the data as a JTAG ICE mkII encapsulated ISP packet. Unlike what AVR067
 * says, the packet gets a length of our response buffer prepended, and replies
 * with RSP_SPI_DATA if successful.
 */
static int stk500v2_jtagmkII_send(const PROGRAMMER *pgm, unsigned char *data, size_t len) {
  ...
  if(sz == SZ_READ_FLASH_EE) {
    /*
     * For CMND_READ_FLASH_ISP and CMND_READ_EEPROM_ISP, extract the size of
     * the return data from the request.  Note that the request itself has the
     * size in big endian format, while we are supposed to deliver it in little
     * endian.
     */
...
static int stk500v2_recv_mk2(const PROGRAMMER *pgm, unsigned char *msg, size_t maxsize) {
  int rv;
...
static int stk500v2_jtagmkII_recv(const PROGRAMMER *pgm, unsigned char *msg, size_t maxsize) {
  int rv;
...
static int stk500v2_jtag3_recv(const PROGRAMMER *pgm, unsigned char *msg, size_t maxsize) {
  int rv;
  unsigned char *jtagmsg;
   ...
   * implementation of JTAGICE3, as they always request a full 512 octets from
   * the ICE.  Thus, only complain at high verbose levels.
   */
  if((size_t) rv - 1 > maxsize) {
...
static int stk500v2_command(const PROGRAMMER *pgm, unsigned char *buf, size_t len, size_t maxlen) {
  int tries = 0;
  int status;
...
retry:
  ...
  if(status > 0) {
    ...
    if(buf[0] == CMD_XPROG_SETMODE || buf[0] == CMD_XPROG) {
      ...
      /*
       * For CMD_XPROG_SETMODE, the status is returned in buf[1]. For
       * CMD_XPROG, buf[1] contains the XPRG_CMD_* command, and buf[2] contains
       * the status.
       */
      ...
      if(buf[1] >= STATUS_CMD_TOUT && buf[1] < 0xa0) {
        ...
        case STATUS_SET_PARAM_MISSING:
          msg = "The `Set Device Parameters' have not been " "executed in advance of this command";
          break;
...
static int stk500v2_cmd(const PROGRAMMER *pgm, const unsigned char *cmd, unsigned char *res) {
  unsigned char buf[8];
  int result;
...
static int stk500v2_jtag3_cmd(const PROGRAMMER *pgm, const unsigned char *cmd, unsigned char *res) {
...
/*
 * Max length of returned message is the sum of all the description strings in
 * the table below, plus 2 characters for separation each. Currently, this is
 * 74 chars plus the terminating nul.
 */
...
static int stk500v2_initialize(const PROGRAMMER *pgm, const AVRPART *p) {
  ...
  /*
   * Examine the avrpart's memory definitions, and initialize the page caches.
...
static int stk500v2_jtag3_initialize(const PROGRAMMER *pgm, const AVRPART *p) {
  ...
  // FIXME: how does condition below make sense?
...
static int stk500hv_read_byte(const PROGRAMMER *pgm, const AVRPART *p, const AVRMEM *mem,
  unsigned long addr, unsigned char *value, enum hvmode mode) {
  int result, cmdlen = 2;
  ...
  if(mem_is_flash(mem)) {
     ...
     * indication to STK500 that a load extended address must be executed.
     */
  ...
  /*
   * In HV mode, we have to use paged reads for flash and EEPROM, and cache the
   * results in a page cache.
   *
 ...
 *
 * By now, only used on the JTAGICE3 which does not implement the CMD_SPI_MULTI
 * SPI passthrough command.
 */
...
static int stk500hv_write_byte(const PROGRAMMER *pgm, const AVRPART *p, const AVRMEM *mem,
  unsigned long addr, unsigned char data, enum hvmode mode) {
  ...
  /*
   * In HV mode, we have to use paged writes for flash and EEPROM.  As both,
   * flash and EEPROM cells can only be programmed from `1' to `0' bits (even
   * EEPROM does not support auto-erase in parallel mode), we just pre-fill the
   * page cache with 0xff, so all those cells that are outside our current
  ...
  if(pagesize) {
    ...
    /*
     * Synthesize the mode byte.  This is simpler than adding yet another
     * parameter to the avrdude.conf file.  We calculate the bits corresponding
     * to the page size, as explained in AVR068. We set bit 7, to indicate this
     * is to actually write the page to the target device.  We set bit 6 to
     * indicate this is the very last page to be programmed, whatever this
     * means -- we just pretend we don't know any better. ;-)  Bit 0 is set if
     * this is a paged memory, which means it has a page size of more than 2.
     */
    buf[3] = 0x80 | 0x40;
    if(pagesize > 2) {
...
static int stk500isp_write_byte(const PROGRAMMER *pgm, const AVRPART *p, const AVRMEM *mem,
  ...
  if(mem_is_flash(mem) || mem_is_eeprom(mem)) {
    ...
    /*
     * We use paged writes for flash and EEPROM, reading back the current page
     * first, modify the byte to write, and write out the entire page.
     */
    if(stk500v2_paged_load(pgm, p, mem, pagesize, paddr, pagesize) < 0)
      return -1;
   ...
   *
   * After all, this is just an ugly hack working around some brokeness in the
   * Atmel firmware starting with the AVRISPmkII (the old JTAGICEmkII isn't
   * affected).  Let's hope 10 ms of additional delay are good enough for
   * everyone.
   */
  usleep(10000);
...
static int stk500hv_paged_write(const PROGRAMMER *pgm, const AVRPART *p, const AVRMEM *m,
  unsigned int page_size, unsigned int addr, unsigned int n_bytes, enum hvmode mode) {
  unsigned int block_size, last_addr, addrshift, use_ext_addr;
  unsigned int maxaddr = addr + n_bytes;
   ...
   * to the page size, as explained in AVR068.  We set bit 7, to indicate this
   * is to actually write the page to the target device.  We set bit 6 to
   * indicate this is the very last page to be programmed, whatever this means
   * -- we just pretend we don't know any better. ;-)  Finally, we set bit 0 to
   * say this is a paged memory, after all, that's why we got here at all.
   */
...
static int stk500v2_paged_load(const PROGRAMMER *pgm, const AVRPART *p, const AVRMEM *m,
  ...
  for(; addr < maxaddr; addr += page_size) {
    ...
    // Ensure load extended address will be issued when crossing a 64 KB boundary in flash
...
// The list of SCK frequencies supported by the AVRISP mkII, as listed in AVR069
 ...
 *
 * This algorithm only fits for the STK500 itself.  For the (old) AVRISP, the
 * resulting ISP clock is only half.  While this would be easy to fix in the
 * algorithm, we'd need to add another configuration flag for this to the
 * config file.  Given the old AVRISP devices are virtually no longer around
 * (and the AVRISPmkII uses a different algorithm below), it's probably not
 * worth the hassle.
 *
 * Originally, magic constants based on the XTAL frequency 7.3728 MHz of the
 * original STK500 HW were used here.
 */
static int stk500v2_set_sck_period(const PROGRAMMER *pgm, double v) {
  unsigned int d;
  unsigned char dur;
...
static void stk500v2_display(const PROGRAMMER *pgm, const char *p) {
  unsigned char maj = 0, min = 0, hdw = 0, topcard = 0, maj_s1 = 0, min_s1 = 0, maj_s2 = 0, min_s2 = 0;
  unsigned int rev = 0;
  ...
  if(my.pgmtype == PGMTYPE_STK500) {
    ...
    msg_info("%sSocket card           : %s\n", p,
      stk600_get_cardname(socket_cards, sizeof socket_cards/sizeof socket_cards[0], topcard));
...
/*
 * Wrapper functions for the JTAG ICE mkII in ISP mode.  This mode uses the
 * normal JTAG ICE mkII packet stream to communicate with the ICE, but then
 * encapsulates AVRISP mkII commands using CMND_ISP_PACKET.
 */
...
static int stk500v2_jtagmkII_open(PROGRAMMER *pgm, const char *port) {
   ...
   * attaching.  If the config file or command-line parameters specify a higher
   * baud rate, we switch to it later on, after establishing the connection
   * with the ICE.
   */
...
/*
 * Wrapper functions for the AVR Dragon in ISP mode.  This mode uses the normal
 * JTAG ICE mkII packet stream to communicate with the ICE, but then
 * encapsulates AVRISP mkII commands using CMND_ISP_PACKET.
 */
...
/*
 * Wrapper functions for the JTAGICE3 in ISP mode.  This mode uses the normal
 * JTAGICE3 packet stream to communicate with the ICE, but then encapsulates
 * AVRISP mkII commands using scope AVRISP.
 */
...
static int stk600_xprog_program_enable(const PROGRAMMER *pgm, const AVRPART *p) {
  ...
  if(use_tpi) {
    // Whatever all that might mean, it matches what AVR Studio does
    ...
    nvm_base = p->nvm_base;
    /*
     * The 0x01000000 appears to be an indication to the programmer that the
     * respective address is located in IO (i.e., SRAM) memory address space
     * rather than flash.  This is not documented anywhere in AVR079 but
     * matches what AVR Studio does.
     */
    nvm_base |= 0x01000000;
    buf[2] = nvm_base >> 24;
    buf[3] = nvm_base >> 16;
...
static int stk600_xprog_paged_load(const PROGRAMMER *pgm, const AVRPART *p, const AVRMEM *mem,
  ...
  /*
   * Fancy offsets everywhere. This is probably what AVR079 means when writing
   * about the "TIF address space".
   */
  if(mem_is_flash(mem)) {
    mtype = 0;
    dynamic_mtype = 1;
    ...
    // @@@ Should consider flash size instead to decide whether to use extended addressing
    if(mem->size > 64*1024)
      use_ext_addr = (1UL << 31);
  } else if(mem_is_eeprom(mem)) {
...
static int stk600_xprog_paged_write(const PROGRAMMER *pgm, const AVRPART *p, const AVRMEM *mem,
  ...
  while(n_bytes != 0) {
    ...
    if(page_size > 256) {
      /*
       * AVR079 is not quite clear.  While it suggests that downloading up to
       * 512 bytes (256 words) were OK, it obviously isn't -- 512-byte pages on
       * the ATxmega128A1 are getting corrupted when written as a single piece.
       * It writes random junk somewhere beyond byte 256. Splitting it into 256
       * byte chunks, and only setting the erase page/write page bits in the
       * final chunk helps.
       */
      if(page_size%256 != 0) {
      ...
      if(n_bytes < page_size) {
        // This can easily happen if the input file was not a multiple of the page size
...
static int stk600_xprog_page_erase(const PROGRAMMER *pgm, const AVRPART *p, const AVRMEM *m, unsigned int addr) {
  unsigned char b[6];

  if(mem_is_flash(m)) {
    b[1] = stk600_xprog_mtype(pgm, addr) == XPRG_MEM_TYPE_APPL? XPRG_ERASE_APP_PAGE: XPRG_ERASE_BOOT_PAGE;
