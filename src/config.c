...
Component avr_comp[] = {
  ...
  part_comp_desc(deviceid_addr, COMP_INT),
  part_comp_desc(deviceid_mask, COMP_INT),
  part_comp_desc(deviceid_expected, COMP_INT),
  part_comp_desc(config_mask0, COMP_INT),
  part_comp_desc(config_mask1, COMP_INT),
  part_comp_desc(config_mask2, COMP_INT),
  part_comp_desc(config_mask3, COMP_INT),
  part_comp_desc(inst_bits, COMP_INT),
 ...
 * Permissive for some invalid unicode sequences but not for those with high
 * bit set). Returns numbers of characters written (0-6).
 */
...
/*
 * Return the next unicode character from a utf-8 string str with at least n
 * characters and record the length of the utf-8 string eaten in *lenp. Returns
 * U+FFFD (illegal char) if parsing does not go well
*/
...
const char *cfg_comp_type(int type) {
  ...
  case COMP_STRING:
    return "string";
  case COMP_CHAR_ARRAY:
    return "byte array";
  case COMP_INT_LISTID:
    return "number list";
  case COMP_STRING_LISTID:
    return "string list";
  case COMP_OPCODE:
    return "opcode";
  case COMP_PIN:
