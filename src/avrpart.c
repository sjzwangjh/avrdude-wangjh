 ...
 * address; addr must be a word address for flash or, for all other memories, a
 * byte address; returns 0 on success and -1 on error (no memory or no opcode)
 * or, if positive, bn+1 where bn is bit number of the highest necessary bit
 * that the opcode does not provide.
 */
 ...
 * avr_set_input()
 *
 * Set input data bits in the specified command based on the opcode, and the
 * data byte.
 */
 ...
 * avr_get_output()
 *
 * Retrieve output data bits from the command results based on the opcode data.
 */
 ...
 *
 * Calculate the byte number of the output data based on the opcode data.
 */
...
// Allocate and initialize memory buffers for each of the device's defined memory regions
...
/*
 * Return pointer to a register that uniquely matches the argument reg or NULL
 * if no or more than one register matches the reg argument.
 *
 * Register names have the form module.name or module.instance.name. The caller
 * provides a matching function which can be str_eq, str_starts, str_matched_by
 * etc. If reg is a full, existing register name, eg, porta.out then a pointer
 * to that register entry is returned irrespective of the matching function.
 * avr_locate_register() also tries to match the last colon-separated segments
 * (instance.name or name) using the provided matching function. If reg is the
 * same as instance.name or name then the matching function switches to
 * str_eq(). This allows the only ADC register adc.adc to be addressed by adc
 * under a lax str_begins() matching even though there are other registers that
 * start with adc, eg, adc.adcsra.
 */
 ...
 * Register names have the form module.name or module.instance.name. The caller
 * provides a matching function which can be str_eq, str_starts, str_matched_by
 * etc. If reg is a full, existing register name, eg, porta.out then the
 * returned list is confined to this specific entry irrespective of the
 * matching function. avr_locate_registerlist() also tries to match the last
 * colon-separated segments (instance.name or name) using the provided matching
 * function. If the argument reg is the same as instance.name or name then the
 * matching function switches to str_eq() reducing the returned list to those
 * that match that full segment. This behaviour can be suppressed by specifying
 * a pattern for reg, eg, adc* together with the matching function
 * str_matched_by.
 */
...
void avr_mem_display(FILE *f, const PROGRAMMER *pgm, const AVRPART *p, const char *prefix) {
  ...
  for(LNODEID ln = lfirst(p->mem); ln; ln = lnext(ln)) {
    ...
    for(int i = 0; i < 4; i++) {
      int len =
        i == memory_col? (int) strlen(avr_mem_name(p, m)):
  ...
  if(p->prog_modes & (PM_PDI | PM_UPDI)) {
    fprintf(f,
      ...
      prefix,
      m_char_max[0], table_padding, m_char_max[1], table_padding, m_char_max[2],
      table_padding, m_char_max[3], table_padding);
  } else {
    fprintf(f,
      "\n%s%-*s  %*s  %-*s\n"
      "%s%.*s--%.*s--%.*s\n",
      ...
      m_char_max[1], table_colum[1],
      m_char_max[2], table_colum[2],
      prefix, m_char_max[0], table_padding, m_char_max[1], table_padding,
  ...
  for(LNODEID ln = lfirst(p->mem); ln; ln = lnext(ln)) {
    ...
    if(p->prog_modes & (PM_PDI | PM_UPDI)) {
      fprintf(f, "%s%-*s  %*d  %*d  %*s \n",
        ...
        m_char_max[1], m->size, m_char_max[2], m->page_size,
        m_char_max[3], print_num("0x%04x", m->offset));
    } else {
      fprintf(f, "%s%-*s  %*d  %*d\n",
        prefix, m_char_max[0], avr_mem_name(p, m), m_char_max[1],
        m->size, m_char_max[2], m->page_size);
    }
  }
}
...
int avr_variants_display(FILE *f, const AVRPART *p, const char *prefix) {
  ...
  if(lsize(p->variants)) {
    ...
    fprintf(f,
      ...
      var_tok_len[1], table_padding, var_tok_len[2], table_padding,
      var_tok_len[3], table_padding, var_tok_len[4], table_padding);
    ...
    for(LNODEID ln = lfirst(p->variants); ln; ln = lnext(ln))
      if(5 == sscanf(ldata(ln), "%49[^:]: %49[^,], Fmax=%49[^,], T=%48[^]]], Vcc=%48[^]]]",
        ...
        fprintf(f,
          ...
          var_tok_len[1], var_tok[1], var_tok_len[2], var_tok[2], var_tok_len[3],
          var_tok[3], var_tok_len[4], var_tok[4]);
      }
...
/*
 * Iterate over the list of avrparts given as "avrparts", and call the callback
 * function cb for each entry found. cb is being passed the following arguments:
 *  - Name of the avrpart (for -p)
 *  - Descriptive text given in the config file
 *  - Name of the config file this avrpart has been defined in
 *  - Line number of the config file this avrpart has been defined at
 *  - Cookie passed into walk_avrparts() (opaque client data)
 */
