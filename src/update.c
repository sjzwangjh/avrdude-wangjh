...
UPDATE *parse_op(const char *s) {
  ...
  if(!is_generated_fname(fn) && len > 2 && fn[len - 2] == ':') { // Format specifier
...
int update_is_readable(const char *fn) {
  ...
  if(is_generated_fname(fn))    // Will be autogenerated
    return 1;
...
static int update_all_from_file(const UPDATE *upd, const PROGRAMMER *pgm, const AVRPART *p,
  ...
  if(is_generated_fname(upd->filename)) // Autogeneration prints its own errors
    if(allsize <= 0)
      return allsize;
